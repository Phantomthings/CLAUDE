"""Rapport des tensions EVI lors des erreurs 84/moment 7.

Ce script extrait toutes les charges présentant l'erreur EVI 84 au moment 7,
analyse les tensions de sortie EVSE dans InfluxDB et génère un fichier Excel
résumant les valeurs min/max et un commentaire décrivant le profil du signal.
"""
...
def fetch_charges(engine, start: Optional[datetime], end: Optional[datetime]) -> pd.DataFrame:
    ...
    charge_id_column = None
    for candidate in ("ID", "Id Charge", "Charge ID", "Id Session"):
        if candidate in available_columns:
            charge_id_column = candidate
            break

    if charge_id_column:
        optional_columns.append(f"`{charge_id_column}` AS `Charge ID`")
    ...
    if "Charge ID" not in df.columns:
        df["Charge ID"] = pd.NA
